...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int cte_cost = 2000;
    const int epsi_cost = 2000;
    const int v_cost = 1;
    const int delta_cost = 10;
    const int a_cost = 10;
    const int delta_change_cost = 100;
    const int a_change_cost = 10;
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += delta_change_cost*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += a_change_cost*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < N; i++) {
    final.push_back(solution.x[x_start + i]);
    final.push_back(solution.x[y_start + i]);
  }
